...
## Project addendum (Doorman-specific, per §2)

The user supplied a screenshot of their real Asana dark-theme list view as the governing
reference for the **project preview area** (`src/components/AsanaProject.tsx`). That region
must remain a pixel-faithful Asana clone — palette `#1e1f21`/`#252628`, Asana enum-pill
colors, grid columns, section carets, "Add task…" rows. Do not "improve" it toward these
standards; fidelity to Asana *is* the design there. These standards apply in full to
everything Doorman-branded around it: sidebar, empty/import state, source panel, future pages.
Display face: **Space Grotesk** (Doorman chrome only — never inside the Asana clone region).
Body: Inter. Room codes: JetBrains Mono, always.

**Tone: this is an internal tool, not a public product.** No marketing headlines, slogans,
taglines, hero-thesis copy, or landing-page voice anywhere. Screen titles say what the screen
does ("Add rooms"), body copy is short plain instruction, and reference material is labeled as
reference. If a sentence would fit on a SaaS homepage, delete or rewrite it. This includes
reassurance flourishes ("…flagged for review, not guessed", "—the project appears the moment…"):
state the fact once, plainly, or not at all.
